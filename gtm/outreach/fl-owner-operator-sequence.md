# Cold Outreach Sequence — FL Owner-Operators
**Target:** Industrial/mixed-use owner-operators, 3–30 assets, Florida
**From:** Ian Baron
**Goal:** Book a 20-minute demo call
**Cadence:** Touch 1 (Email) → Touch 2 (LinkedIn DM, Day 3) → Touch 3 (Email, Day 7)

---

## Touch 1 — Email (Day 0)

**Subject:** Your insurance bill, [City] industrial

**Body:**

[First name],

Quick question — when did you last retender your commercial insurance across the portfolio?

Most owner-operators I talk to in Florida are sitting on 25–35% overpay vs what's actually available in market right now. On a 5–10 asset portfolio that's typically $18k–$40k a year just sitting on the table.

I run Arca. We audit your insurance, energy, and rent roll against live market benchmarks, then go execute the savings. Commission-only — we earn a percentage of what we save you, nothing if we don't deliver.

Worth a 20-minute look at the numbers? I'll pull your portfolio data before the call so we're not wasting time.

Ian

---

## Touch 2 — LinkedIn DM (Day 3)

Hi [First name] — sent you a note a couple days ago about your commercial portfolio.

One specific thing: Florida energy rates have moved significantly in the last 18 months. Operators on legacy contracts are typically paying 15–20% above what's available on a new commercial deal right now.

Happy to show you where you sit vs market — takes 20 minutes. No obligation.

Still the right person to talk to about this?

---

## Touch 3 — Email (Day 7)

**Subject:** Re: Your insurance bill, [City] industrial

**Body:**

[First name],

Last one from me.

We recently ran a portfolio health check for a Florida mixed-use operator — 8 assets, similar profile to yours. Found:

- $22k/yr insurance overpay (placed with two new carriers, saved 28%)
- $11k/yr energy savings (switched commercial tariff, live in 3 weeks)
- Two missed income streams (EV charging + subletting opportunity on one asset)

Total year-1 uplift: ~$41k. Our commission: a fraction of that. Their net: the rest.

If the timing's wrong, no problem. But if you want to see what that looks like for your portfolio specifically, I'm easy to reach.

Ian Baron
Arca
[phone]
https://propra-app-production.up.railway.app/book?name=[FIRST+LAST]&company=[COMPANY]&assets=[N]

---

## Outreach Link Personalization

Replace the `/book` link in Touch 3 with a personalized URL for each prospect.

**Format:** `https://propra-app-production.up.railway.app/book?name=[FIRST+LAST]&company=[COMPANY+NAME]&assets=[N]`

**Example:** `https://propra-app-production.up.railway.app/book?name=John+Smith&company=Smith+Industrial+LLC&assets=8`

**What it does:**
- Shows the prospect a personalized booking page (their name + company pre-filled)
- Pre-fills the cal.com embed with their name
- Fires a lead capture event to Arca — **Ian gets an email notification when the prospect clicks the link**
- Tracks which prospects engage with the outreach

**Fastest way to generate links:** Go to `/admin/leads` → OutreachLinkGen tool → enter name, company, asset count → copies the URL ready to paste.

**Do not use** the bare `cal.com/arca/demo` link — it gives no lead capture and no personalization.

---

## LinkedIn Connection Request (use if no email available)

Hi [First name] — I work with Florida commercial property owners to find savings and income they're leaving on the table (insurance, energy, rent roll, ancillary). Commission-only, no fees. Would value connecting.

---

## Targeting Notes

**Where to find them:**
- CoStar/LoopNet ownership records for FL industrial and mixed-use
- LinkedIn: search "commercial property" + "Florida" + "owner" / "portfolio" / "investments"
- County property appraiser records (public, searchable by owner for FL)
- Local BOMA Florida chapter membership lists
- NAIOP Florida chapter (members tend to be active investors, not just brokers)

**Prioritise:**
- Owners with 5–15 assets (sweet spot for our commission math)
- Assets held 3+ years (more likely on legacy insurance/energy contracts)
- Mixed-use and industrial (higher energy intensity = bigger energy savings opportunity)
- No institutional management company listed (means they're doing it themselves = our target)
