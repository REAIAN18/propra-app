        ...
        <div className="rounded-xl p-6" style={{ backgroundColor: "#0d1825", border: "1px solid #1a2d45" }}>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {portfolios.map((p) => {
                const shareLink = `${process.env.NEXT_PUBLIC_APP_URL ?? (typeof window !== "undefined" ? window.location.origin : "https://realhq.com")}/dashboard?portfolio=${p.urlKey}&welcome=1&company=${encodeURIComponent(p.name)}`;
                return (
                  <div
                    key={p.id}
