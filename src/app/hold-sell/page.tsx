...
/* ── Data source badge ─────────────────────────────────────────────────── */
const srcBadgeColors: Record<string, React.CSSProperties> = {
...
const srcBadgeTitles: Record<string, string> = {
  avm: "Automated valuation model (ATTOM)",
  lease: "Extracted from lease data",
  user: "Your manually entered assumption",
  market: "Market benchmark data",
};
function SrcBadge({ type }: { type: "avm" | "lease" | "user" | "market" }) {
  return (
    <span
      title={srcBadgeTitles[type]}
      style={{
        display: "inline-flex",
        alignItems: "center",
        padding: "1px 5px",
...
export default function HoldSellPage() {
  const [saleActioned, setSaleActioned] = useState<Map<string, string>>(new Map());
      ...
      <main className="flex-1 overflow-y-auto" style={{ background: "var(--bg)", padding: "28px 32px 80px" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
                  ...
                  return (
                    <div key={scenario.assetId} style={{ borderBottom: "1px solid var(--bdr-lt)" }}>
                      ...
                      >
                        ...
                        <div style={{ textAlign: "right" }}>
                          <div style={{ font: "500 8px/1 var(--mono)", color: "var(--tx3)", textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 3 }}>
                            Exit value<SrcBadge type="avm" />
                          </div>
                      ...
                      <div style={{ padding: "0 18px 14px" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 10 }}>
                          {([
                            { label: "Hold IRR", value: `${scenario.holdIRR}%`, color: "var(--grn)", badge: "market" as const },
                            { label: "Exit IRR", value: `${scenario.sellIRR}%`, color: scenario.recommendation === "sell" ? "var(--red)" : "var(--amb)", badge: "market" as const },
                            { label: "IRR Gain", value: `${irrDiff > 0 ? "+" : ""}${irrDiff.toFixed(1)}pp`, color: irrDiff > 0 ? "var(--amb)" : "var(--grn)", badge: "market" as const },
                          ] as { label: string; value: string; color: string; badge: "avm" | "lease" | "user" | "market" }[]).map(m => (
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          {scenario.recommendation === "sell" && (
                            saleActioned.has(scenario.assetId) ? (
                              <Link
                                href={`/portal/${saleActioned.get(scenario.assetId)}`}
                              ...
                              <button
                                onClick={async () => {
                                  const res = await fetch("/api/user/transactions", {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({ type: "disposal", assetId: scenario.assetId, askingPrice: sellPrice }),
                                  }).catch(() => null);
                                  const data = res?.ok ? await res.json().catch(() => null) : null;
                                  const roomId = data?.room?.id ?? "new";
                                  setSaleActioned(prev => new Map([...prev, [scenario.assetId, roomId]]));
                                }}
          ...
          {!loading && sellCandidates.length > 0 && (() => {
            ...
            const avmValue = sellAsset.estimatedValue ?? 0;
            const jurisdiction = portfolio.currency === "GBP" ? "UK" : "US";
            // Compute illustrative gain: sale price vs current AVM value
            const hasGainData = sp > 0 && avmValue > 0;
            const estimatedGain = hasGainData ? sp - avmValue : null;
            const taxRate = jurisdiction === "UK" ? 0.24 : 0.20;
            const estimatedTax = estimatedGain != null && estimatedGain > 0 ? Math.round(estimatedGain * taxRate) : null;
            const deferralAmount = estimatedTax; // 1031 defers the full tax
            const annualExemption = jurisdiction === "UK" ? 3000 : 0;
            const taxableGain = estimatedGain != null ? Math.max(0, estimatedGain - annualExemption) : null;
            const taxAfterExemption = taxableGain != null ? Math.round(taxableGain * taxRate) : null;
            return (
              <>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ padding: 18 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                      <div>
                        <div style={{ font: "500 8px/1 var(--mono)", color: "var(--tx3)", textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 4 }}>
                          Current AVM Value<SrcBadge type="avm" />
                        </div>
                        <div style={{ fontFamily: "var(--serif)", fontSize: 17, color: avmValue > 0 ? "var(--tx)" : "var(--tx3)" }}>
                          {avmValue > 0 ? fmt(avmValue, sym) : "—"}
                        </div>
                        {avmValue === 0 && <div style={{ font: "400 10px var(--sans)", color: "var(--acc)", marginTop: 2 }}>Add asset value to calculate</div>}
                      </div>
                      <div>
                        <div style={{ font: "500 8px/1 var(--mono)", color: "var(--tx3)", textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 4 }}>
                          Estimated Sale Price<SrcBadge type="avm" />
                        </div>
                        <div style={{ fontFamily: "var(--serif)", fontSize: 17, color: "var(--tx)" }}>{fmt(sp, sym)}</div>
                      </div>
                      <div>
                        <div style={{ font: "500 8px/1 var(--mono)", color: "var(--tx3)", textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 4 }}>Estimated Gain vs AVM</div>
                        <div style={{ fontFamily: "var(--serif)", fontSize: 17, color: estimatedGain != null && estimatedGain > 0 ? "var(--grn)" : estimatedGain != null ? "var(--red)" : "var(--tx3)" }}>
                          {estimatedGain != null ? (estimatedGain >= 0 ? "+" : "") + fmt(Math.abs(estimatedGain), sym) : "—"}
                        </div>
                      </div>
                      <div>
                        <div style={{ font: "500 8px/1 var(--mono)", color: "var(--tx3)", textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 4 }}>
                        ...
                        <div style={{ fontFamily: "var(--serif)", fontSize: 17, color: taxAfterExemption != null && taxAfterExemption > 0 ? "var(--red)" : "var(--tx3)" }}>
                          {taxAfterExemption != null && taxAfterExemption > 0 ? fmt(taxAfterExemption, sym) : "—"}
                        </div>
                        {jurisdiction === "UK" && annualExemption > 0 && estimatedGain != null && (
                          <div style={{ font: "400 10px var(--sans)", color: "var(--tx3)", marginTop: 2 }}>After £{annualExemption.toLocaleString()} annual exemption</div>
                        )}
                      </div>
                    </div>
                    ...
                    {jurisdiction === "US" && (
                      <div style={{ padding: 14, background: "var(--grn-lt)", border: "1px solid var(--grn-bdr)", borderRadius: 8, marginBottom: 12 }}>
                        ...
                        <div style={{ font: "300 12px/1.6 var(--sans)", color: "var(--tx2)" }}>
                          Reinvest the full {fmt(sp, sym)} proceeds into a like-kind property within{" "}
                          <strong style={{ color: "var(--tx)" }}>180 days</strong> to defer capital gains tax
                          {deferralAmount != null && deferralAmount > 0 ? ` (up to ${fmt(deferralAmount, sym)} deferred)` : ""}.
                    ...
                    <div style={{ padding: "10px 14px", background: "var(--s2)", border: "1px solid var(--bdr)", borderRadius: 8, font: "300 11px/1.5 var(--sans)", color: "var(--tx3)" }}>
                      ⚠ This is an illustrative estimate only. Gain is computed vs current AVM value, not acquisition cost.
                      Actual tax liability depends on your individual circumstances,
