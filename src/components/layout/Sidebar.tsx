...
function NavBadge({ count, variant = "gray" }: { count: number; variant?: "green" | "amber" | "red" | "gray" }) {
  ...
  const styles: Record<string, string> = {
    green: "background:var(--grn-lt);color:var(--grn)",
    amber: "background:var(--amb-lt);color:var(--amb)",
    red: "background:var(--red-lt);color:var(--red)",
    gray: "background:var(--s2);color:var(--tx3)",
  };
  return (
    <span
...
function LivePill() {
  return (
    <span className="ml-auto flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: "var(--grn-lt)", color: "var(--grn)" }}>
      <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: "var(--grn)" }} />
      LIVE
    </span>
  );
}
...
function NewPill() {
  return (
    <span className="ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: "var(--acc-lt)", color: "var(--acc)" }}>
      New
    </span>
  );
}
...
function SavingPill({ text, color }: { text: string; color: "green" | "teal" }) {
  const bg = color === "green" ? "var(--grn-lt)" : "var(--grn-lt)";
  const fg = color === "green" ? "var(--grn)" : "var(--grn)";
  return (
...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  const sections: NavSection[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          badgeVariant: "gray",
          icon: <Ico d="M2 11V5.5L7 2.5l5 3V11M4.5 7H6V11H4.5zM7.5 7H9V11H7.5z" />,
        },
        {
          href: "/scout",
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          badge: alerts.income,
          badgeVariant: "green",
          icon: <svg width="13" height="13" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="7" cy="7" r="5"/><path d="M5 7h4M7 5v4"/></svg>,
        },
        {
          href: "/income",
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          icon: <Ico d="M1.5 2.5h11v9h-11zM4.5 2.5V1.5M9.5 2.5V1.5M1.5 6h11" />,
        },
      ],
    },
    {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="flex items-center justify-between px-3.5 py-3.5" style={{ borderBottom: "1px solid var(--bdr-lt)" }}>
        <div className="flex items-center gap-2.5">
          <div
            className="w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold leading-none flex-shrink-0"
            style={{ backgroundColor: "var(--grn)", color: "var(--bg)", letterSpacing: "-0.5px" }}
          >
            R
          </div>
          <div>
            <div className="text-sm font-bold" style={{ color: "var(--tx)", letterSpacing: "-0.3px" }}>RealHQ</div>
            <div className="text-[10px]" style={{ color: "var(--tx2)" }}>{portfolio.shortName}</div>
          </div>
        </div>
        {onClose && (
          <button
              ...
              {section.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    style={{
                      color: active ? "var(--grn)" : "var(--tx2)",
                      fontWeight: active ? 600 : 400,
                      opacity: active ? 1 : 0.7,
                      backgroundColor: active ? "var(--grn-lt)" : "transparent",
                      border: active ? "1px solid var(--grn-bdr)" : "1px solid transparent",
                    }}
      ...
      {/* Footer */}
      <div className="px-3.5 py-3" style={{ borderTop: "1px solid var(--bdr-lt)" }}>
