...
export async function searchGazetteByCompanyName(
  ...
  try {
    ...
    return results.map((notice: Record<string, unknown>) => ({
      ...
      noticeType: extractNoticeType((notice.title || "") as string),
      publicationDate: ((notice.publication_date || notice.date_published) as string),
      category: ((notice.category || "Insolvency") as string),
      url: ((notice.url || notice.gazette_url) as string | undefined),
    }));
  } catch (error) {
